...
def terminal_prediction(y_hat, y_actual, test_terminals, total_solution_edges):
        ...
        for j in range(len(y_hat[i])):
           if j in test_terminals[i][0]:
             ...
             if y_hat[i][j]==1:
               ...
               TN += 1
        total_terminals += len(test_terminals[i][0])
    print("total_terminals:", total_terminals)
    print("TP, FP, TN, FN", TP, FP, TN, FN)
    total_steiner_nodes = 0
    for i, sol_edges in enumerate(total_solution_edges):
      sol_nodes = set()
      for u, v, w in sol_edges:
        sol_nodes.add(u)
        sol_nodes.add(v)
      sol_nodes = list(sol_nodes)
      total_steiner_nodes += (len(sol_nodes)-len(test_terminals[i][0]))
    print("total_steiner_nodes:", total_steiner_nodes)
    print("steiner_nodes/terminals:", total_steiner_nodes/total_terminals)
...
for i, name in enumerate(test_file_names):
 ...
 for j in range(n_lines):
  #f.write(str(v) + '\n')
  l = f.readline()
  cur_pred.append(float(l))
 f.close()
 prediction.append(cur_pred)
terminal_prediction(prediction, test_labels, test_terminals, test_solution_edges)
...
def generate_solution_graphs(prediction, test_graph_edges, test_solution_edges, test_terminals):
 ...
 print("maximum ratio:", max(rat_arr))
 print("average ratio:", avg_rat)
 print("minimum ratio:", min(rat_arr))

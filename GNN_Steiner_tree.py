...
for k in range(len(graph_nodes)):
  for p in range(number_of_graphs_with_same_setting):
    ...
    E1, N1, answer1 = load_graph_to_matrix("./Steiner_data/"+experiment_folder+"/graph_ER_"+str(graph_nodes[len(graph_nodes)-1])+"_1_L_"+str(node_size)+"_"+str(p)+"_train_node_weighted.txt",k*len(graph_nodes)+p)
    E1 = np.asarray(E1)
    N1 = np.asarray(N1)
...
for k in range(len(graph_nodes)):
  for p in range(number_of_graphs_with_same_setting):
    node_size = graph_nodes[k]
    E, N, answer = load_graph_to_matrix("./Steiner_data/"+experiment_folder+"/graph_ER_"+str(graph_nodes[len(graph_nodes)-1])+"_1_L_"+str(node_size)+"_"+str(p)+"_test_node_weighted.txt",k*len(graph_nodes)+p)
    E = np.asarray(E)
    N = np.asarray(N)
    #print("E:", E)
